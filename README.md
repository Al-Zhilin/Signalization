...
В каталоге с проектом вы можете увидеть файл `passwords_example.txt`, который содержит все конфиденциальные и самые важные дефайны. Остальная их часть находиться в файле Signalization.ino. Чтобы проект успешно загрузился и работал, вам нужно:

- Заполнить все конфиденциальные данные в дефайнах файла `passwords_example.txt` (**ssid** и **password**, **BOT_TOKEN** и так далее...)
- Переименовать файл из `passwords_example.txt` в `passwordse.txt`. Это позволит проекту видеть ваши настройки
...
- В разработке система перезагрузки роутера/модема при потере интернет-соединения
