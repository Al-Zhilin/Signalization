...
- Заполнить все конфиденциальные данные в дефайнах файла `passwords_example.h` (**ssid** и **password**, **BOT_TOKEN** и так далее...) вашими значениями
- Переименовать файл из `passwords_example.h` в `passwords.h`. Это позволит проекту видеть ваши настройки и успешно их использовать
...
Копируем эту строку, вплоть до `&p1=` в файл с настройками `passwords.h`, а именно в этот дефайн: `#define MONITORING ""` 
...
- v1.0 - Релизная версия. Все основные функции реализованы
- v1.1:
  - Исправлен и улучшен алгоритм фильтрации сработок дачтиков (движения и пожарных)
  - Исправлен алгоритм автовключения датчиков
  - Глобально переработана система работы с температурными датчиками
  - Добавлены несколько проверок на адекватность пользовательских значений в дефайнах
