void newMsg(FB_msg& msg) {
  ...
  if (msg.text == "Ок")   digitalWrite(r_pins[MODEM_RELE_NUM-1], LOW);
  if (msg.text == "Оке")   digitalWrite(r_pins[MODEM_RELE_NUM-1], HIGH);
  ...
  if (msg.text == "/res") {
    ...
    bot.sendMessage("Между сработками - " + String(EEPROM.read(3)) + " секунд", msg.chatID);
    bot.sendMessage("Между автовключениями датчиков - " + String(EEPROM.read(2)) + " минут", msg.chatID);
  ...
  else if (msg.OTA && (msg.text == "update" || msg.fileName == "Signalization_7.0.ino.bin")) {
